...
class PurchaseOrder(models.Model):
    ...
    def _compute_receipt_percentage(self):
        ...
        for order in with_lines:
            lines_percentages = order.order_line.mapped("receipt_percentage")
            perc = sum(lines_percentages) / len(lines_percentages)
            order.update(
                {
                    "receipt_percentage": perc,
                    ...
                    "receipt_percentage_display": perc / 100,
                }
            )
