...
class MergePurchaseAutomatic(models.TransientModel):
    ...
    def _check_content(self, purchase_orders):
        ...
        currency_names = set(purchase_orders.mapped(lambda o: o.currency_id.name))
        picking_type_names = set(
            purchase_orders.mapped(lambda o: o.picking_type_id.name)
        )
        incoterm_names = set(purchase_orders.mapped(lambda o: o.incoterm_id.name))
        ...
        if len(currency_names) > 1:
            error_msg += "{} {}: {}\n".format(
                start_msg, _("currencies"), ", ".join([str(x) for x in currency_names])
            )
        if len(picking_type_names) > 1:
            error_msg += "{} {}: {}\n".format(
                start_msg,
                _("picking types"),
                ", ".join([str(x) for x in picking_type_names]),
            )
        if len(incoterm_names) > 1:
            error_msg += "{} {}: {}\n".format(
                start_msg, _("incoterms"), ", ".join([str(x) for x in incoterm_names])
            )
        if len(payment_term_names) > 1:
            error_msg += "{} {}: {}\n".format(
                start_msg,
                _("payment terms"),
                ", ".join([str(x) for x in payment_term_names]),
            )
        if len(fiscal_position_names) > 1:
            error_msg += "{} {}: {}\n".format(
                start_msg,
                _("fiscal positions"),
                ", ".join([str(x) for x in fiscal_position_names]),
            )
