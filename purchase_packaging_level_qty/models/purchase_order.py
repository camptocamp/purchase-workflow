...
class PurchaseOrder(models.Model):
    ...
    def _compute_total_transport_packaging_qty(self):
        for order in self:
            plevel = order._get_purchase_transport_packaging_level()
            if plevel:
...
class PurchaseOrderLine(models.Model):
    ...
    def _get_transport_packaging_qty(self):
        plevel = self.order_id._get_purchase_transport_packaging_level()
        product_packaging = self.product_id.packaging_ids.filtered(
            lambda self: self.packaging_level_id == plevel
        )
    ...
    def _compute_line_transport_qty(self):
        for line in self:
            ...
            plevel = line.order_id._get_purchase_transport_packaging_level()
            transport_packaging_qty = line._get_transport_packaging_qty()
            if plevel and transport_packaging_qty > 0:
