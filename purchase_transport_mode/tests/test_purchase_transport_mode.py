...
class TestPurchaseTransportMode(TransactionCase):
    ...
    def enable_purchase_transaport_mode_validation(self):
        self.settings = self.env["res.config.settings"].create({})
        self.settings.check_purchase_transport_mode_contraints = True
        self.settings.execute()

    def test_disabled_purchase_transaport_mode_validation(self):
        self.purchase_order.button_confirm()
        self.assertEqual(self.purchase_order.state, "purchase")

    def test_purchase_transaport_mode_validation(self):
        self.enable_purchase_transaport_mode_validation()
