...
class PurchaseOrder(models.Model):
    ...
    def _compute_transport_mode_validation_status(self):
        for rec in self:
            ...
            rec.transport_mode_status_ok = rec.transport_mode_status == {}

    @api.onchange("partner_id")
    def onchange_partner_id(self):
        if self.partner_id and not self.transport_mode_id:
            self.transport_mode_id = self.partner_id.purchase_transport_mode_id

    def _get_transport_mode_validation_status(self):
        self.ensure_one()
        errors = []
        if not self.company_id.check_purchase_transport_mode_contraints:
            return {}
